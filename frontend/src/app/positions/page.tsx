...
interface Position {
  ...
  symbol: string;
  qty: number;        // lot / adet
  buyPrice: number;   // alış fiyatı (USD)
...
const INITIAL_CAPITAL = 5000; // USD
const STORAGE_KEY = "finance_positions_v1";

const SYMBOL_META: Record<string, { name: string; color: string; flag?: string }> = {
  ...
  YEOTK:     { name: "Yeo Tech",   color: "#FF9F0A" },
...
export default function PositionsPage() {
  ...
  /* ── Fetch live prices for all used symbols ── */
  ...
  function openAdd() {
    setEditId(null);
    setForm({ symbol: "XAU/USD", qty: "", buyPrice: "", buyDate: new Date().toISOString().split("T")[0], note: "" });
    setShowForm(true);
  }

  function openEdit(p: Position) {
    setEditId(p.id);
    setForm({ symbol: p.symbol, qty: String(p.qty), buyPrice: String(p.buyPrice), buyDate: p.buyDate, note: p.note });
    setShowForm(true);
  }
  ...
  function saveForm() {
    ...
    setPositions((prev) =>
      editId ? prev.map((p) => (p.id === editId ? pos : p)) : [...prev, pos]
    );
    setShowForm(false);
  }
      ...
      <main className="max-w-5xl mx-auto px-5 py-8 space-y-6">
        ...
        {positions.length === 0 ? (
          <div className="rounded-[16px] py-16 flex flex-col items-center gap-3"
            style={{ background: "#1C1C1E", border: "0.5px solid rgba(255,255,255,0.07)" }}>
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#3A3A3C" strokeWidth="1.5" strokeLinecap="round">
              <path d="M12 5v14M5 12h14"/>
            </svg>
            <p className="text-[14px]" style={{ color: "#48484A" }}>Henüz pozisyon yok</p>
            <button onClick={openAdd}
              className="text-[12px] px-4 py-1.5 rounded-[8px]"
            ...
            <table className="w-full text-[13px] border-collapse">
              ...
              <tbody>
                {positions.map((p, i) => {
                  ...
                  const meta = SYMBOL_META[p.symbol] ?? { name: p.symbol, color: "#AEAEB2" };
                  ...
                  return (
                    <tr key={p.id}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-[10px]" style={{ color: "#636366" }}>{p.buyDate}{p.note ? ` · ${p.note}` : ""}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div>
                          <p className="font-semibold font-mono" style={{ color: pnl >= 0 ? "#30D158" : "#FF453A" }}>
                            {pnl >= 0 ? "+" : ""}{fmt(pnl)} $
                          </p>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(p)}
                            className="p-1.5 rounded-md hover:bg-white/10 transition-all" title="Düzenle">
                          ...
                          <button onClick={() => deletePos(p.id)}
                            className="p-1.5 rounded-md hover:bg-red-500/20 transition-all" title="Sil">
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#636366" strokeWidth="2" strokeLinecap="round">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-md rounded-[20px] p-6 space-y-4"
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-semibold uppercase tracking-wide" style={{ color: "#636366" }}>Sembol</label>
              <div className="flex flex-wrap gap-1.5">
                {SYMBOLS.map((s) => (
                  <button key={s} onClick={() => setForm((f) => ({ ...f, symbol: s }))}
                    className="px-2.5 py-1 rounded-[7px] text-[12px] font-medium transition-all"
                    style={{
                      background: form.symbol === s ? SYMBOL_META[s]?.color + "33" : "#2C2C2E",
                      color: form.symbol === s ? SYMBOL_META[s]?.color : "#AEAEB2",
                      border: `0.5px solid ${form.symbol === s ? SYMBOL_META[s]?.color + "66" : "transparent"}`,
                    }}>
                    {s}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <input type="number" step="any" placeholder="0.01"
            ...
            {form.qty && form.buyPrice && (
              <div className="px-3 py-2 rounded-[9px]" style={{ background: "rgba(88,86,214,0.1)", border: "0.5px solid rgba(88,86,214,0.2)" }}>
                <p className="text-[12px]" style={{ color: "#A29BFF" }}>
                  Toplam maliyet: <span className="font-bold">${fmt(parseFloat(form.qty) * parseFloat(form.buyPrice))}</span>
                  {cashLeft - parseFloat(form.qty) * parseFloat(form.buyPrice) < 0 && (
                    <span className="ml-2" style={{ color: "#FF9F0A" }}>⚠️ Nakit yetersiz</span>
                  )}
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <input type="text" placeholder="serbest not…"
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={saveForm}
                disabled={!form.qty || !form.buyPrice}
                className="flex-1 py-2 rounded-[9px] text-[13px] font-semibold transition-all"
                style={{ background: !form.qty || !form.buyPrice ? "#2C2C2E" : "#5856D6", color: !form.qty || !form.buyPrice ? "#636366" : "#fff" }}>
                {editId ? "Kaydet" : "Ekle"}
              </button>
            </div>
          </div>
